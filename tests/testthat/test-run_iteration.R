...
test_that("run_iteration throws error when data_source_run argument is missing (no default value provided)", {
  expect_error(
    run_iteration(
      data_source_run = ,
      bin_selection = "first",
...
test_that("run_iteration throws error when data_source_run is NULL (invalid empty input)", {
  expect_error(
    run_iteration(
...
# character
test_that("run_iteration throws error when data_source_run is a character string ('my_data') instead of required data structure", {
  expect_error(
    run_iteration(
...
# Numeric
test_that("run_iteration throws error when data_source_run is a numeric value (123) instead of required data structure", {
  expect_error(
    run_iteration(
      data_source_run = 123,
      bin_selection = "first",
...
# Zero
test_that("run_iteration throws error when data_source_run is zero (numeric 0) instead of required data structure", {
  expect_error(
    run_iteration(
      data_source_run = 0,
      bin_selection = "first",
...
# NA
test_that("run_iteration throws error when data_source_run is NA (missing value) instead of required data structure", {
  expect_error(
    run_iteration(
      data_source_run = NA,
      bin_selection = "first",
...
# empty list
test_that("run_iteration throws error when data_source_run is an empty list (list() with no elements)", {
  expect_error(
    run_iteration(
...
# empty dataframe
test_that("run_iteration throws error when data_source_run is an empty data frame (data.frame() with no rows/columns)", {
  expect_error(
    run_iteration(
...
test_that("run_iteration throws error when bin_selection is an invalid string ('my_choice') not among allowed options ('first', 'random', 'last')", {
  suppressWarnings(
    data_to_run_bins <-
      extract_data(
        data_community_extract = RRatepol::example_data$pollen_data[[
          1
        ]],
...
test_that("run_iteration throws error when bin_selection is a numeric value (1) instead of required character string", {
  suppressWarnings(
    data_to_run_bins <-
      extract_data(
        data_community_extract = RRatepol::example_data$pollen_data[[
...
test_that("run_iteration throws error when bin_selection contains multiple values ('first', 'random') instead of a single option", {
  suppressWarnings(
    data_to_run_bins <-
      extract_data(
        data_community_extract = RRatepol::example_data$pollen_data[[
...
test_that("run_iteration throws error when bin_selection contains multiple values in different order ('random', 'first') instead of a single option", {
  suppressWarnings(
    data_to_run_bins <-
      extract_data(
        data_community_extract = RRatepol::example_data$pollen_data[[
...
# bin_selection = NULL
test_that("run_iteration throws error when bin_selection is NULL instead of a valid selection option", {
  suppressWarnings(
    data_to_run_bins <-
      extract_data(
        data_community_extract = RRatepol::example_data$pollen_data[[
...
# standardise = NULL
test_that("run_iteration throws error with NULL in standardise parameter", {
  suppressWarnings(
    data_to_run_bins <-
      extract_data(
        data_community_extract = RRatepol::example_data$pollen_data[[
...
test_that("run_iteration treats non-boolean value for standardise as FALSE", {
  suppressWarnings(
    data_to_run_bins <-
      extract_data(
        data_community_extract = RRatepol::example_data$pollen_data[[
...
test_that("run_iteration throws error when n_individuals is zero with standardise=TRUE (cannot standardize with zero individuals)", {
  suppressWarnings(
    data_to_run_bins <-
      extract_data(
        data_community_extract = RRatepol::example_data$pollen_data[[
...
test_that("run_iteration throws warning for invalid combinations of standardise=TRUE with n_individuals=NULL", {
  suppressWarnings(
    data_to_run_bins <-
      extract_data(
        data_community_extract = RRatepol::example_data$pollen_data[[
...
test_that("run_iteration throws error for non-boolian tranform_to_proportions = 0", {
  suppressWarnings(
    data_to_run_bins <-
      extract_data(
        data_community_extract = RRatepol::example_data$pollen_data[[
...
# )
# NULL
test_that("run_iteration throws error when dissimilarity_coefficient is NULL instead of valid coefficient option", {
  suppressWarnings(
    data_to_run_bins <-
      extract_data(
        data_community_extract = RRatepol::example_data$pollen_data[[
...
# Character
test_that("run_iteration throws error when dissimilarity_coefficient is an invalid string ('my_choice') not among allowed options", {
  suppressWarnings(
    data_to_run_bins <-
      extract_data(
        data_community_extract = RRatepol::example_data$pollen_data[[
...
# Numeric
test_that("run_iteration throws error when dissimilarity_coefficient is a numeric value (123) instead of valid character string", {
  suppressWarnings(
    data_to_run_bins <-
      extract_data(
        data_community_extract = RRatepol::example_data$pollen_data[[
...
# Multiple
test_that("run_iteration throws error when dissimilarity_coefficient contains multiple values ('euc', 'euc.sd') instead of single option", {
  suppressWarnings(
    data_to_run_bins <-
      extract_data(
        data_community_extract = RRatepol::example_data$pollen_data[[
...
# Zero
test_that("run_iteration throws error when dissimilarity_coefficient is zero (numeric 0) instead of valid character string", {
  suppressWarnings(
    data_to_run_bins <-
      extract_data(
        data_community_extract = RRatepol::example_data$pollen_data[[
...
# NA
test_that("run_iteration throws error when dissimilarity_coefficient is NA (missing value) instead of valid coefficient option", {
  suppressWarnings(
    data_to_run_bins <-
      extract_data(
        data_community_extract = RRatepol::example_data$pollen_data[[
...
test_that("run_iteration treats non-boolean value for tranform_to_proportions as FALSE", {
  suppressWarnings(
    data_to_run_bins <-
      extract_data(
        data_community_extract = RRatepol::example_data$pollen_data[[
...
test_that("run_iteration handles boolean TRUE for time_standardisation as 1", {
  suppressWarnings(
    data_to_run_bins <-
      extract_data(
        data_community_extract = RRatepol::example_data$pollen_data[[
...
# with verbose = FALSE
test_that("run_iteration throws no message if standardisation failed if verbose = FALSE", {
  suppressWarnings(
    data_to_run_bins <-
      extract_data(
        data_community_extract = RRatepol::example_data$pollen_data[[
...
# tranform_to_proportions is only checked if TRUE. Anything else will be regarded as FALSE.
test_that("run_iteration throws error with numeric zero for tranform_to_proportions", {
  suppressWarnings(
    data_to_run_bins <-
      extract_data(
        data_community_extract = RRatepol::example_data$pollen_data[[
...
test_that("run_iteration issues a warning if NULL is supplied for verbose", {
  suppressWarnings(
    data_to_run_bins <-
      extract_data(
        data_community_extract = RRatepol::example_data$pollen_data[[
...
test_that("run_iteration issues a warning if NA is supplied for verbose", {
  suppressWarnings(
    data_to_run_bins <-
      extract_data(
        data_community_extract = RRatepol::example_data$pollen_data[[
...
test_that("run_iteration output has expected structure and column names", {
  suppressWarnings(
    data_to_run_bins <-
      extract_data(
        data_community_extract = RRatepol::example_data$pollen_data[[
...
test_that("run_iteration output has non-NA values in expected columns", {
  suppressWarnings(
    data_to_run_bins <-
      extract_data(
        data_community_extract = RRatepol::example_data$pollen_data[[
...
test_that("run_iteration RoC values are numeric and positive", {
  suppressWarnings(
    data_to_run_bins <-
      extract_data(
        data_community_extract = RRatepol::example_data$pollen_data[[
...
test_that("run_iteration with different time_standardisation gives proportional RoC values", {
  suppressWarnings(
    data_to_run_bins <-
      extract_data(
        data_community_extract = RRatepol::example_data$pollen_data[[
...
test_that("run_iteration with standardise=TRUE produces valid output", {
  suppressWarnings(
    data_to_run_bins <-
      extract_data(
        data_community_extract = RRatepol::example_data$pollen_data[[
...
test_that("run_iteration with different dissimilarity_coefficient produces different RoC values", {
  suppressWarnings(
    data_to_run_bins <-
      extract_data(
        data_community_extract = RRatepol::example_data$pollen_data[[
...
test_that("run_iteration output is deterministic with fixed seed for random bin_selection", {
  suppressWarnings(
    data_to_run_bins <-
      extract_data(
        data_community_extract = RRatepol::example_data$pollen_data[[
...
test_that("run_iteration output has valid label column", {
  suppressWarnings(
    data_to_run_bins <-
      extract_data(
        data_community_extract = RRatepol::example_data$pollen_data[[
...
test_that("run_iteration handles very small time_standardisation correctly", {
  suppressWarnings(
    data_to_run_bins <-
      extract_data(
        data_community_extract = RRatepol::example_data$pollen_data[[
...
test_that("run_iteration output age values match input data range", {
  suppressWarnings(
    data_to_run_bins <-
      extract_data(
        data_community_extract = RRatepol::example_data$pollen_data[[
  ...
  expect_true(
    min(result$res_age) >= min_input_age ||
      abs(min(result$res_age) - min_input_age) < 500
  )
  expect_true(
    max(result$res_age) <= max_input_age ||
      abs(max(result$res_age) - max_input_age) < 500
  )
})

test_that("run_iteration output has reasonable RoC values for typical inputs", {
  suppressWarnings(
    data_to_run_bins <-
      extract_data(
        data_community_extract = RRatepol::example_data$pollen_data[[
...
test_that("run_iteration executes successfully with all valid parameters", {
  suppressWarnings(
    data_to_run_bins <-
      extract_data(
        data_community_extract = RRatepol::example_data$pollen_data[[
