...
test_that("extract_data() throws error if verbose is character", {
  expect_error(
    extract_data(
      data_community_extract = RRatepol::example_data$pollen_data[[1]],
      data_age_extract = RRatepol::example_data$sample_age[[1]],
...
test_that("extract_data() throws error if verbose is numeric", {
  expect_error(
    extract_data(
      data_community_extract = RRatepol::example_data$pollen_data[[1]],
      data_age_extract = RRatepol::example_data$sample_age[[1]],
...
test_that("extract_data() throws error if verbose is NULL", {
  expect_error(
    extract_data(
      data_community_extract = RRatepol::example_data$pollen_data[[1]],
      data_age_extract = RRatepol::example_data$sample_age[[1]],
...
test_that("extract_data() produces start message when verbose = TRUE", {
  expect_message(
    extract_data(
      data_community_extract = RRatepol::example_data$pollen_data[[1]],
      data_age_extract = RRatepol::example_data$sample_age[[1]],
...
test_that("extract_data() produces completion message when verbose = TRUE", {
  expect_message(
    extract_data(
      data_community_extract = RRatepol::example_data$pollen_data[[1]],
      data_age_extract = RRatepol::example_data$sample_age[[1]],
...
test_that("extract_data() produces no messages when verbose = FALSE and no data issues", {
  expect_silent(
    extract_data(
      data_community_extract = RRatepol::example_data$pollen_data[[1]],
      data_age_extract = RRatepol::example_data$sample_age[[1]],
...
test_that("extract_data fails if data_community_extract is matrix", {
  expect_error(
    extract_data(
      data_community_extract = as.matrix(RRatepol::example_data$pollen_data[[
        1
      ]]),
      data_age_extract = RRatepol::example_data$sample_age[[1]],
      age_uncertainty = NULL
...
test_that("extract_data() handles community data with character columns - returns error", {
  char_community <-
    RRatepol::example_data$pollen_data[[1]] %>%
    sapply(., as.character) %>%
    as.data.frame()
...
test_that("extract_data() throws error if sample_id is numeric in community data", {
...
test_that("extract_data() throws error if sample_id is factor in community data", {
  community_factor_id <-
    RRatepol::example_data$pollen_data[[1]]
...
test_that("extract_data() throws error if sample_id is numeric in age data", {
  age_numeric_id <-
    RRatepol::example_data$sample_age[[1]]
...
test_that("extract_data() throws error if age column is missing in age data", {
  age_no_age_col <-
    RRatepol::example_data$sample_age[[1]] %>%
    dplyr::select(-age)
...
test_that("extract_data() throws error if age column is character", {
  age_char_age <-
    RRatepol::example_data$sample_age[[1]]
  age_char_age$age <-
...
test_that("extract_data() throws error if age column is factor", {
  age_factor_age <-
    RRatepol::example_data$sample_age[[1]]
  age_factor_age$age <-
...
test_that("extract_data() throws error if age_uncertainty has wrong number of columns", {
  age_un_wrong_size <-
...
test_that("extract_data() throws error if age_uncertainty has too many columns", {
  age_un_extra <-
    cbind(
      RRatepol::example_data$age_uncertainty[[1]],
...
test_that("extract_data() throws error if age_uncertainty is data.frame instead of matrix", {
  age_un_df <-
...
test_that("extract_data() throws error if age_uncertainty is character vector", {
  expect_error(
    extract_data(
      data_community_extract = RRatepol::example_data$pollen_data[[1]],
      data_age_extract = RRatepol::example_data$sample_age[[1]],
...
test_that("extract_data() throws error if community has additional samples (rows)", {
...
test_that("extract_data() throws error if age has more rows than community data", {
  extra_age <-
    rbind(
      RRatepol::example_data$sample_age[[1]],
...
test_that("extract_data() throws error if data comunity and age is empty", {
  empty_community <-
...
test_that("extract_data() handles single row data - returns list", {
  single_community <-
    RRatepol::example_data$pollen_data[[1]][1, ]
  single_age <-
    RRatepol::example_data$sample_age[[1]][1, ]
...
test_that("extract_data() handles single row data - returns named list", {
  single_community <-
    RRatepol::example_data$pollen_data[[1]][1, ]
  single_age <-
    RRatepol::example_data$sample_age[[1]][1, ]
...
test_that("extract_data() handles single row data - community has 1 row", {
  single_community <-
    RRatepol::example_data$pollen_data[[1]][1, ]
  single_age <-
    RRatepol::example_data$sample_age[[1]][1, ]
...
test_that("extract_data() handles single row data - age has 1 row", {
  single_community <-
    RRatepol::example_data$pollen_data[[1]][1, ]
  single_age <-
    RRatepol::example_data$sample_age[[1]][1, ]
...
test_that("extract_data() handles minimal community data - returns list", {
  minimal_community <-
...
test_that("extract_data() throws error if community has 0 columns", {
  minimal_community <-
    RRatepol::example_data$pollen_data[[1]][, 1, drop = FALSE]
...
test_that("extract_data() throws error with duplicate sample_id in community data", {
  dup_community <-
    RRatepol::example_data$pollen_data[[1]]
...
test_that("extract_data() sorts unsorted age data", {
  unsorted_age <-
    RRatepol::example_data$sample_age[[1]]
...
test_that("extract_data() maintains sorted age data", {
  sorted_age <-
    RRatepol::example_data$sample_age[[1]] %>%
    dplyr::arrange(age)
...
test_that("extract_data() handles partial NA in age data - produces message", {
  partial_na_age <-
    RRatepol::example_data$sample_age[[1]]
...
test_that("extract_data() handles some NA in age data - filters out NA rows", {
  partial_na_age <-
    RRatepol::example_data$sample_age[[1]]
...
test_that("extract_data() handles partial NA in age data - no remaining NAs", {
  partial_na_age <-
    RRatepol::example_data$sample_age[[1]]
  partial_na_age$age[1:3] <-
    NA
...
test_that("extract_data() returns correct structure with minimal data - returns list", {
  minimal_community <-
...
test_that("extract_data() returns correct structure with minimal data - named correctly", {
  minimal_community <-
    RRatepol::example_data$pollen_data[[1]][3, 1:2]
...
test_that("extract_data() returns correct structure with minimal data - community is data.frame", {
  minimal_community <-
    RRatepol::example_data$pollen_data[[1]][3, 1:2]
...
test_that("extract_data() returns correct structure with minimal data - age is data.frame", {
  minimal_community <-
    RRatepol::example_data$pollen_data[[1]][3, 1:2]
...
test_that("extract_data() returns correct structure with minimal data - age_un is NULL", {
  minimal_community <-
    RRatepol::example_data$pollen_data[[1]][3, 1:2]
...
test_that("extract_data() returns correct structure with minimal data - community rownames", {
  minimal_community <-
    RRatepol::example_data$pollen_data[[1]][3, 1:2]
...
test_that("extract_data() returns correct structure with minimal data - age rownames", {
  minimal_community <-
    RRatepol::example_data$pollen_data[[1]][3, 1:2]
...
test_that("extract_data() returns correct structure with minimal data - community values", {
  minimal_community <-
    RRatepol::example_data$pollen_data[[1]][3, 1:2]
  ...
  expect_equal(
    result$community$`Chenopodiaceae/Amaranthaceae`,
    minimal_community[2]$`Chenopodiaceae/Amaranthaceae`
  )
})

## 12.1i Test output structure with minimal valid data - age values correct
test_that("extract_data() returns correct structure with minimal data - age values", {
  minimal_community <-
    RRatepol::example_data$pollen_data[[1]][3, 1:2]
...
test_that("extract_data() correctly assigns column names to age uncertainty", {
  result <-
    extract_data(
      data_community_extract = RRatepol::example_data$pollen_data[[1]],
      data_age_extract = RRatepol::example_data$sample_age[[1]],
