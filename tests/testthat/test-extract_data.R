...
test_that(
  "extract_data() throws error if community has additional samples (rows)", {
    ...
    extra_sample_in_community$sample_id[nrow(extra_sample_in_community)] <- 
      "EXTRA_SAMPLE"
...
test_that(
  "extract_data() handles minimal community data - community has 0 columns", {
    ...
    expect_equal(
      ncol(
        result$community), 0) # after removing sample_id column
  })
...
test_that(
  "extract_data() sorts unsorted age data",
  {
    ...
    unsorted_age <-
      unsorted_age[order(runif(nrow(unsorted_age))), ] # randomize order
    
    rownames(unsorted_age) <- NULL
...
test_that("extract_data() handles all NA community data - replaces with zeros & returns empty result", {
...
test_that(
  "extract_data() handles some NA in age data - filters out NA rows", {
    ...
    n_na <- sum(is.na(partial_na_age$age))
